...
<div class="panel panel-default">
    ...
    <h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.unit_currency}</h3> 
            ...
            <div class="col-xs-24 col-sm-12  col-md-12  col-lg-8">
                <div class="form-group">
                    <input class="form-control" type="text" value="{Q}" name="q" maxlength="255"
                        placeholder="Tên đơn vị tiền tệ, ký hiệu" />
                </div>
            </div>
        ...
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    ...
                    <th>{LANG.name_currency}</th>
					<th>{LANG.symbol}</th>
                    <th>{LANG.exchange_currency}</th>
                    <th>{LANG.time_add}</th>
                    <th>{LANG.user_add}</th>
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.name_currency}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="name_currency" value="{ROW.name_currency}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
	<div class="form-group">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.exchange_currency}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
...
//<![CDATA[
    function nv_change_weight(id) {
        ...
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_currency&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
            var r_split = res.split('_');
            if (r_split[0] != 'OK') {
                alert(nv_is_change_act_confirm[2]);
            }
            window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_currency';
            return;
        });
        return;
    }
    ...
    function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_currency&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
                var r_split = res.split('_');
                if (r_split[0] != 'OK') {
                    alert(nv_is_change_act_confirm[2]);
                }
