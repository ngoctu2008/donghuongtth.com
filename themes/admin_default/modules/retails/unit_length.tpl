...
<div class="panel panel-default">
    ...
    <h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.unit_length}</h3> 
            ...
            <div class="col-xs-24 col-sm-12  col-md-12  col-lg-8">
                <div class="form-group">
                    <input class="form-control" type="text" value="{Q}" name="q" maxlength="255"
                        placeholder="Tên đơn vị khối lượng, ký hiệu" />
                </div>
            </div>
...
<form action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    <th class="w100">{LANG.length}</th>
                    <th>{LANG.name_length}</th>
					<th>{LANG.symbol}</th>
                    <th>{LANG.exchange}</th>
                    <th>{LANG.time_add}</th>
                    <th>{LANG.user_add}</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <select class="form-control" id="id_length_{VIEW.id}" onchange="nv_change_length('{VIEW.id}');">
                        <!-- BEGIN: length_loop -->
                            <option value="{WEIGHT.key}"{WEIGHT.selected}>{WEIGHT.title}</option>
                        <!-- END: length_loop -->
                    </select>
                </td>
                    <td> {VIEW.name_length} </td>
					<td> {VIEW.symbol} </td>
                    <td> {VIEW.exchange} </td>
                    <td> {VIEW.time_add} </td>
                    <td> {VIEW.user_add} </td>
...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
    ...
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.name_length}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="name_length" value="{ROW.name_length}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
	<div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.symbol}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="symbol" value="{ROW.symbol}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-5 col-md-4 control-label"><strong>{LANG.exchange}</strong> <span class="red">(*)</span></label>
        <div class="col-sm-19 col-md-20">
            <input class="form-control" type="text" name="exchange" value="{ROW.exchange}" required="required" oninvalid="setCustomValidity(nv_required)" oninput="setCustomValidity('')" />
        </div>
    </div>
...
//<![CDATA[
    function nv_change_length(id) {
        var nv_timer = nv_settimeout_disable('id_length_' + id, 5000);
        var new_vid = $('#id_length_' + id).val();
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_length&nocache=' + new Date().getTime(), 'ajax_action=1&id=' + id + '&new_vid=' + new_vid, function(res) {
            var r_split = res.split('_');
            if (r_split[0] != 'OK') {
                alert(nv_is_change_act_confirm[2]);
            }
            window.location.href = script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_length';
            return;
        });
        return;
    }
    ...
    function nv_change_status(id) {
        ...
        if (confirm(nv_is_change_act_confirm[0])) {
            ...
            $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=unit_length&nocache=' + new Date().getTime(), 'change_status=1&id='+id, function(res) {
                var r_split = res.split('_');
                if (r_split[0] != 'OK') {
                    alert(nv_is_change_act_confirm[2]);
                }
