...
$dataContent['total_product'] = $db->query('SELECT count(*) FROM ' . TABLE . '_order_item WHERE order_id = ' . (int)$order_id )->fetchColumn();
$dataContent['total_weight']=number_format($order_info['total_weight']);
$dataContent['barecode'] = barcode($order_info['shipping_code']);
$dataContent['barecode_1'] = barcode($order_info['district_id']);
$dataContent['logo'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/transporters/'.$db->query('SELECT image FROM ' . TABLE . '_transporters where id='.$order_info['transporters_id'])->fetchColumn();

$xtpl = new XTemplate( 'order_print.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
