...
if (!nv_function_exists('nvb_landing_product_freeship')) {
	/**
	* @param string $module
	...
	* @return string
	*/
	function nvb_config_landing_product_freeship($module, $data_block, $lang_block)
	{
	...
	* @return number
	*/
	function nvb_config_landing_product_freeship_submit($module, $lang_block)
	{
		global $nv_Request;
	...
	* @return string
	*/
	function nvb_landing_product_freeship($block_config)
	{
		...
		if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $block_config['module'] . '/global.product_freeship.tpl')) {
			$block_theme = $global_config['module_theme'];
		} elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $block_config['module'] . '/global.product_freeship.tpl')) {
			$block_theme = $global_config['site_theme'];
		} else {
			$block_theme = 'default';
		}
		
		$sql = 'SELECT t1.id,t1.image,t1.alias,t1.name_product,t1.star,t1.price,t1.price_special, free_ship FROM ' .$db_config['dbsystem']. '.'. NV_PREFIXLANG . '_' . $block_config['module'] . '_product t1 INNER JOIN ' .$db_config['dbsystem']. '.'. NV_PREFIXLANG . '_' . $block_config['module'] . '_block_id t2 ON t1.id = t2.product_id  WHERE t1.free_ship = 1 AND t1.inhome = 1 AND t2.bid = ' . $block_config['group_id'] . ' ORDER BY rand() limit ' . $block_config['number_item'];
		
		
		$list = $db->query($sql)->fetchAll();

		$xtpl = new XTemplate('global.product_freeship.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $block_config['module']);
...
if (defined('NV_SYSTEM')) {
	$content = nvb_landing_product_freeship($block_config);
}
