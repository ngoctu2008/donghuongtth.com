...
$array_notification = array(
	'0' => 'Không thông báo',
	'1' => 'Thông báo tất cả',
	'2' => 'Thông báo đến cửa hàng',
	'3' => 'Thông báo đến khách hàng'
);
...
if ($is_submit_form) {
	...
	$rowcontent['notification'] = $nv_Request->get_int('notification', 'post', 0);
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $copy) {
            ...
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows (
                catid, listcatid, notification, topicid, admin_id, author, sourceid, addtime, edittime, status, weight, publtime, exptime, archive, title, alias, hometext,
                ...
                instant_creatauto
                 ...
                 :listcatid,
				 ' . intval($rowcontent['notification']) . ',
                 ' . $rowcontent['topicid'] . ',
            ...
            $sth = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_rows SET
                ...
                listcatid=:listcatid,
                notification=' . $rowcontent['notification'] . ',
...
// danh sách thông báo
foreach($array_notification as $index => $value)
{
	$selected = ($index == $rowcontent['notification']) ? 'selected=selected' : '';
	
	$xtpl->assign('selected_notification', $selected);
	$xtpl->assign('value_notification', $value);
	$xtpl->assign('index_notification', $index);
