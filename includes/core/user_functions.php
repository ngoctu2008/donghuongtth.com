...
function nv_html_meta_tags($html = true)
{
	...
	if (empty($meta_property['og:image'])) {
            $meta_property['og:image'] = NV_MAIN_DOMAIN . NV_BASE_SITEURL . $global_config['site_banner'];
        }
		
		if (empty($meta_property['og:image:alt'])) {
            $meta_property['og:image:alt'] = $site_description;
        }
