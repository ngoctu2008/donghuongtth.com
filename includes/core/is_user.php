        ...
        if (!empty($user) and $global_config['allowuserlogin']) {
            ...
            if (isset($user['userid']) and isset($user['checknum']) and isset($user['checkhash'])) {
                ...
                if ($user['checkhash'] === md5($user['userid'] . $user['checknum'] . $global_config['sitekey'] . $client_info['browser']['key'])) {
                    $_sql = 'SELECT userid, group_id, username, email, first_name, last_name, gender, photo, birthday, regdate,phone,
                        ...
                        last_openid AS current_openid, password, question, answer, safemode, email_verification_time,phone
                    ...
                    if (!empty($user_info)) {
                        ...
                        if ($checknum) {
                            $user_info['full_name'] = nv_show_name_user('', $user_info['last_name'], '');
