...
function email_register($array_register)
{
	global $module_info, $global_config, $lang_global, $lang_module, $module_name, $op, $nv_redirect;
	
	$xtpl = new XTemplate('email_thong_bao_dangky_muahang.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/retails');
	
	$xtpl->assign('LOGO_SRC', $_SERVER["SERVER_NAME"] . '/' . $global_config['site_logo']);
	
	$xtpl->assign('SITE_NAME', $global_config['site_name']);
	
	$xtpl->assign('CONTACT', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=contact',true));
	
	$xtpl->assign('LOGIN', nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=users',true));
	
	$xtpl->assign('USER', $array_register);
...
function user_register($gfx_chk, $checkss, $data_questions, $array_field_config, $custom_fields, $group_id)
{
	...
	$xtpl->assign('validate_phone', '/(0)+([0-9]{9})\b/g');
...
function user_login($is_ajax = false)
{
    ...
    $xtpl->assign('REGISTER', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=register');
...
function send_email_lostpassword($name, $sitename, $key, $pa)
{
	global $module_info, $global_config, $lang_global, $lang_module, $module_name, $op;
	
    $xtpl = new XTemplate('send_email_lostpassword.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_info['module_theme']);
	
	 $xtpl->assign('name', $name);
	 $xtpl->assign('sitename', $sitename);
	 $xtpl->assign('key', $key);
	 $xtpl->assign('pa', date('d/m/Y - H:i',$pa));
...
function user_welcome()
{
	...
	if($_user_info['gender'] == 'M')
	{
		$_user_info['checked_m'] = 'checked=checked';
	}
	elseif($_user_info['gender'] == 'F'){
		$_user_info['checked_f'] = 'checked=checked';
	}
...
function nv_memberslist_detail_theme($item, $array_field_config, $custom_fields)
{
    ...
    $xtpl->assign('URL_MODULE', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name);
	if($item['last_name']){
		$item['full_name'] = nv_show_name_user($item['last_name']);
	}
