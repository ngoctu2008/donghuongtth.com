...
=======
    <div class="row">
        <div class="col-md-5 m-3 d-flex">
            ...
            <div class="productDetail_shop_info ml-3 mt-3">
                <p class="fs_20">{info_shop.company_name}</p>
                <button id="follow_shop_{info_shop.id}" onclick="follow({info_shop.id})"
                    class="{FOLLOW.color_follow}">{FOLLOW.title_follow}</button>
                <input type="number" name="check_{info_shop.id}" value="{FOLLOW.check_follow}" hidden="">
            </div>
        </div>
                ...
                if (check == 0) {
                    $.ajax({
                        ...
                        url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                            nv_fc_variable + '=viewcatshops&mod=follow&id=' + shop_id,
                        success: function(res2) {
                            //res2=JSON.parse(res);
                            //console.log(res2)
                            if (res2.status == "OK") {
                    ...
                    $.ajax({
                        ...
                        dataType: "JSON",
                        url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                            nv_fc_variable + '=viewcatshops&mod=un_follow&id=' + shop_id,
                        success: function(res2) {
                            //res2=JSON.parse(res);
                            if (res2.status == "OK") {
                                $('#follow_shop_' + shop_id).addClass('btn_ecng_outline');
                ...
                <div class="coupons_right">
                    <div class="d-flex flex-column justify-content-around text-center h-100">
                        ...
                        <div>
                            <!-- BEGIN: not_saved -->
                            <div id="voucher_{VOUCHER.id}">
                                <button onclick="save_voucher({VOUCHER.id})"
                                    class="{FOLLOW.color_follow} btn_ecng">Lưu</button>
                            </div>
                            <!-- END: not_saved -->
...
<section class="categoryFilter bg_white py-3 mb-3  d-flex align-items-center ">
    ...
    <div id="price" class="mx-2 d-flex justify-content-between align-items-center rounded position-relative"
        name="categoryfilter_price" style="padding: 5px 0px">
        <div class="mb-0 text_primary pl-2 secondary_text" id="categoryFilter_price" value="0">Giá</div>
        <i class="fa fa-caret-down pr-2 secondary_text" aria-hidden="true"></i>
        <ul id="price_content" class="position-absolute pl-0 bg_white w-100">
            <li class="py-2 pl-2" value="0">Giá</li>
    ...
    <div id="rate" class=" mx-2 d-flex justify-content-between align-items-center rounded position-relative"
