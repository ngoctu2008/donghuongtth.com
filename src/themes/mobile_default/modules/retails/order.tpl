...
<div class="panel panel-default panel_add_thong_tin hidden">
    ...
    <span>
        ...
        <input id="b_email" type="email" name="order_email1" onchange="check_email_error(this)"
            value="{USER_INFO.email}" class="form-control">
    </span>
    <span>
        <p>
...
<section class="shop_bill px-3 pt-2 py-4 mt-2 bg_white">
    ...
    <script>
        ...
        if (giatri["status"] == 1) {
            content +=
                '<button class="fs_12 p-2 text-white border-0 rounded" style="background:#e1a208" onclick="remove_choose_voucher_' +
                store_id + '(' + giatri["voucherid"] + ',' + store_id + ')">Bỏ chọn</button>';
        } else {
            content +=
                '<button class="fs_12 text-white p-2 border-0 rounded" style="background:#e1a208" onclick="apply_voucher_' +
...
<section>
    ...
    <div class="pb-2 pt-3 px-3 bg_white my-2">
        ...
        <img class="rounded" src="{NV_BASE_SITEURL}themes/{TEMPLATE}/chonhagiau/images/icon/icon_vnpay.svg"
            alt="VN PAY">
...
<script>
    function add_voucher(shop_id) {
        ...
        $.ajax({
            type: 'GET',
            url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                nv_fc_variable + '=ajax&mod=add_voucher&voucher_code=' + voucher_code + '&shop_id=' + shop_id +
                '&method=1&voucher_pice_one_shop=' + voucher_pice_one_shop,
            dataType: "json",
            beforeSend: function() {
                $('#apply_' + shop_id).prop('disabled', true);
            },
    ...
    function delete_voucher(shop_id) {
        ...
        $.ajax({
            ...
            url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' +
                nv_fc_variable + '=ajax&mod=add_voucher&voucher_code=' + voucher_code + '&shop_id=' + shop_id +
                '&method=0&voucher_pice_one_shop=' + voucher_pice_one_shop,
            dataType: "json",
            beforeSend: function() {
...
<script>
    ...
    function load_data_tranposter_next(store_id, warehouse_id, transporter_id, name_transporters, description,
        ...
        $("#tranposter_next_" + store_id + "_" + warehouse_id).append(
            '<div class="row mb-10"><div class="col-1"><label class="ecng_label_radio"><input name="carrier[' +
            store_id + '][' + warehouse_id + ']" value=' + transporter_id + ' title="' + name_transporters +
            '" title2="' + description + '"' + onclick + ' ' + checked +
            ' type="radio" name="radio"><span class="checkmark"></span></label></div><div class="col-8"><p class="fs_16 mb-0">' +
            name_transporters + '</p><span class="text_gray_color">' + description +
            '</span></div><div class="col-3 text-right secondary_text"><span id="phivanchuyen_' + store_id + '_' +
            warehouse_id + '_' + transporter_id + '">' + phivanchuyen + ' </span>đ</div></div>');
