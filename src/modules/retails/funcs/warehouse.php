...
if ($nv_Request->isset_request('change_status', 'post, get')) {
    ...
    $query = 'SELECT status FROM ' .$db_config['dbsystem']. '.'. TABLE . '_warehouse WHERE id=' . $id;
    $row = $db->query($query)->fetch();
    if (isset($row['status']))     {
        $status = ($row['status']) ? 0 : 1;
        $query = 'UPDATE ' .$db_config['dbsystem']. '.'. TABLE . '_warehouse SET status=' . intval($status) . ' WHERE id=' . $id;
        $db->query($query);
        $content = 'OK_' . $id;
    }
...
if ( !empty( $q ) ) {
    $where .= ' AND (name_warehouse LIKE "%" "'.$q. '" "%" OR name_send LIKE "%" "'.$q. '" "%" OR phone_send LIKE "%" "'.$q. '")';
    $base_url .= '&q=' . $q;
}
if($store_id>0){
	$where .= ' AND sell_id ='.$store_id;
...
if (!$nv_Request->isset_request('id', 'post,get')) {
    ...
    $db->sqlreset()
        ->select('COUNT(*)')
        ->from('' .$db_config['dbsystem']. '.'. TABLE . '_warehouse')
		->where('1=1'.$where);
	print_r($db->sql());die;
