...
$email = 'trungnghiaweb@gmail.com';
//$email = 'vhoatoi269@gmail.com';
$subject = 'Thông tin khách hàng - Chợ Nhà Giàu';
$message = 'Xin chào trungnghiaweb,

Tài khoản của bạn tại website ECNG | Sàn Thương Mại Điện Tử Chợ Nhà Giàu đã được kích hoạt. Dưới đây là thông tin đăng nhập:

Tên tài khoản: trungnghiaweb

Bạn hãy click vào đường dẫn dưới đây để đăng nhập và đổi mật khẩu:
URL: https://chonhagiau.vn/index.php/users/

Đây là thư tự động được gửi đến hòm thư điện tử của bạn từ website ECNG | Sàn Thương Mại Điện Tử Chợ Nhà Giàu. Nếu bạn không hiểu gì về nội dung bức thư này, đơn giản hãy xóa nó đi.

Quản trị site';

$send = nv_sendmail([$global_config['site_name'], $global_config['site_email']], $email, $subject, $message);
...
function nv_check_email_reg(&$email)
{
    ...
    $stmt->bindParam(':pattern', $email, PDO::PARAM_STR);
    $stmt->execute();
    if ($stmt->fetchColumn()) {
...
if ($checkss == $array_register['checkss']) {
    ...
    $array_register['first_name'] = '';
    $array_register['last_name'] = nv_substr($nv_Request->get_title('name', 'post', '', 1), 0, 255);
    $array_register['username'] = $nv_Request->get_title('phone', 'post', '', 1);
	...
	$array_register['phone'] = $nv_Request->get_title('phone', 'post', '');
	...
	if($array_register['phone']!=''){
		$check = preg_match('/^[0]{1}[0-9]{9}+$/',$array_register['phone']);
		if(empty($check)){
			reg_result(array(
				'status' => 'error',
				'input' => 'phone',
	...
	$db->query('INSERT INTO '. $db_config['dbsystem']. '.'.NV_PREFIXLANG. '_retails_customer(userid,phone,user_add,time_add,status) VALUES('.$userid.','.$db->quote($array_register['phone']).','.$userid.','.NV_CURRENTTIME.',1)');

    require_once NV_ROOTDIR .'/modules/wallet/wallet.class.php';
    $wallet = new nukeviet_wallet();
    $checkUpdate1 = $wallet->init2($userid);
    $message = 'Cộng tiền khởi tạo';

    $checkUpdate = $wallet->update(0, 'VND',$userid, $message,  true);
    if (!$userid) {
        reg_result(array(
            'status' => 'error',
            'input' => '',
    ...
    active, checknum, last_login, last_ip, last_agent, last_openid, idsite, email_verification_time, active_obj,phone
    ) VALUES (
    ...
    '', 0, 1,
    '" . (defined('ACCESS_ADDUS') ? $group_id : ($global_users_config['active_group_newusers'] ? 7 : 4)) . "',
    1, '', 0, '', '', '', " . $global_config['idsite'] . ", -1, 'SYSTEM',:phone
)";
...
$data_insert['birthday'] = intval($array_register['birthday']);
$data_insert['sig'] = $array_register['sig'];
$data_insert['phone'] = $array_register['phone'];
...
if (!$userid) {
	...
	$subject = $lang_module['account_success'];
	$message = email_register($array_register);
