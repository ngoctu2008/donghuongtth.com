...
{
    global $nv_Cache, $global_config, $lang_global, $module_name, $op;

    $list_cats = array();
    $sql = 'SELECT id, parentid, title, link, icon,image, note, subitem, groups_view, module_name, op, target, css, active_type, module_name FROM ' . NV_PREFIXLANG . '_menu_rows WHERE status=1 AND mid = ' . $block_config['menuid'] . ' ORDER BY weight ASC';
    ...
    foreach ($list as $row) {
        if (nv_user_in_groups($row['groups_view'])) {
            ...
            if (!empty($row['image']) and file_exists(NV_UPLOADS_REAL_DIR . '/menu/' . $row['image'])) {
                $row['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/menu/' . $row['image'];
            } else {
                $row['image'] = '';
            }
    ...
    if (!empty($list_cats)) {
        foreach ($list_cats as $cat) {
            if (empty($cat['parentid'])) {
				...
				// xử lý ac active li
				if($module_name == $cat['module_name'])
				{
					$cat['active'] = 'class="active_li_a"';
				}
				else
				{
					$cat['active'] = '';
                ...
                if (!empty($cat['image'])) {
                    $xtpl->assign('IMG', $cat['image']);
                    $xtpl->parse('main.loopcat1.image');
                }
                if (!empty($cat['icon'])) {
                    $xtpl->assign('ICON', $cat['icon']);
