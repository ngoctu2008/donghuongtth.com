...
foreach ($users_list as $u) {
    ...
    if (!defined('NV_IS_USER_FORUM')) {
        ...
        if ($u['is_delete']) {
			$check_seller=$db->query('SELECT count(*) FROM '.$db_config['dbsystem']. '.'.NV_PREFIXLANG. '_retails_seller_management where userid='.$u['userid'])->fetchColumn();
			$check_customer=$db->query('SELECT count(*) FROM '.$db_config['dbsystem']. '.'.NV_PREFIXLANG. '_retails_customer where userid='.$u['userid'])->fetchColumn();
			if($check_seller==0 && $check_customer==0){
