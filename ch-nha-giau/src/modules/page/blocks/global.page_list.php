...
if (!nv_function_exists('nv_page_list')) {
    ...
    function nv_page_list($block_config)
    {
        ...
        $db->sqlreset()->select('id, title, alias, description')->from(NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'])->where('status = 1 AND hot_post =1')->order('weight ASC')->limit($block_config['numrow']);
